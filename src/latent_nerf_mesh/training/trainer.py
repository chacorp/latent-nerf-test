...
from src.latent_nerf_mesh.models.renderer import NeRFRenderer
from src.latent_nerf_mesh.training.nerf_dataset import NeRFDataset
...
class Trainer:
    ...
    def init_nerf(self) -> NeRFRenderer:
        if self.cfg.render.backbone == 'grid':
            from src.latent_nerf_mesh.models.network_grid import NeRFNetwork
        else:
    ...
    def init_mesh_model(self) -> nn.Module:
        if self.cfg.mesh.backbone == 'texture-mesh':
            from src.latent_nerf_mesh.models.textured_mesh import TexturedMeshModel
        ...
        elif self.cfg.mesh.backbone == 'texture-rgb-mesh':
            from src.latent_nerf_mesh.models.textured_mesh import TexturedMeshModel
            model = TexturedMeshModel(self.cfg, device=self.device, render_grid_size=self.cfg.mesh.train_grid_size,
                                      latent_mode=False, texture_resolution=self.cfg.guide.texture_resolution).to(self.device)
        else:
            raise NotImplementedError(f'--backbone {self.cfg.mesh.backbone} is not implemented!')

        model = model.to(self.device)
        logger.info(
            f'Loaded {self.cfg.mesh.backbone} Mesh, #parameters: {sum([p.numel() for p in model.parameters() if p.requires_grad])}')
        logger.info(model)
        return model
    ...
    def init_dataloaders(self) -> Dict[str, DataLoader]:
        train_dataloader = NeRFDataset(self.cfg.render, device=self.device, type='train', 
                                       H=self.cfg.render.train_h, W=self.cfg.render.train_w, 
                                       size=100).dataloader()
        val_loader       = NeRFDataset(self.cfg.render, device=self.device, type='val', 
                                       H=self.cfg.render.eval_h, W=self.cfg.render.eval_w,
                                       size=self.cfg.log.eval_size).dataloader()
        # Will be used for creating the final video
        val_large_loader = NeRFDataset(self.cfg.render, device=self.device, type='val', 
                                       H=self.cfg.render.eval_h, W=self.cfg.render.eval_w,
    ...
    def train(self):
        ...
        while self.train_step < self.cfg.optim.iters:
            ...
            for data in zip(self.dataloaders['train']):
                # TODO:
                # 1. render NeRf (depth, image)
                # 2. render mesh with differentable renderer (depth, image)
                # 3. calculate loss 
                #    3-1. loss between depth images
                #    3-2. loss between RGB images
                # 4. optimize mesh
    ...
    def train_render(self, data: Dict[str, Any]):
        ...
        ## image = 64x64
        outputs_mesh  = self.mesh_model.render(theta=theta, phi=phi, radius=radius)
        pred_rgb_mesh = outputs_mesh['image']
        lap_loss_mesh = outputs_mesh['lap_loss']
        ...
        L1_loss = self.l1_loss(pred_rgb_mesh, pred_rgb)
        
        # render with diffusion
        pred_rgb_vis = self.diffusion.decode_latents(pred_rgb).permute(0, 2, 3, 1).contiguous()  # [1, 3, H, W]
        pred_rgb_mesh_vis = self.diffusion.decode_latents(pred_rgb_mesh).permute(0, 2, 3, 1).contiguous()  # [1, 3, H, W]
        render_L1_loss = self.l1_loss(pred_rgb_mesh_vis, pred_rgb_vis)

...
class TexturedMeshModel(nn.Module):
    def __init__(self,
        ...
        self.dy = self.opt.guide_mesh.dy
        self.mesh_scale = self.opt.guide_mesh.shape_scale
        ...
        self.renderer = Renderer(device=self.device, dim=(render_grid_size, render_grid_size),
                                 interpolation_mode=self.opt.guide_mesh.texture_interpolation_mode)
    ...
    def init_meshes(self, env_sphere_path='shapes/env_sphere.obj'):
        ...
        mesh = Mesh(self.opt.guide_mesh.shape_path, self.device)
